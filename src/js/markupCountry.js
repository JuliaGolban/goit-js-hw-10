...
function createMarkupArticleCountry(data) {
  const markup = data
    ...
    <img class="country-info__img" src="${flags.svg}" alt="${name.official}" width="100">
  ...
  <ul class="country-info__list">
    ...
    <li class="country-info__feature"><b>Languages:</b> ${languages}</li>
  </ul>`;
    })
    .join('');
...
function createMarkupListOfCountries(data) {
  const markup = data
    .map(({ name, flags }) => {
      ...
      <img class="country-info__img" src="${flags.svg}" alt="${name.official} width="40">
